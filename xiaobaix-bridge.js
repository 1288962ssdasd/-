...
(function () {
  ...
  if (window.XBBridge) {
    if (!window.XBBridge.send && window.XBBridge.generate && typeof window.XBBridge.generate.generate === 'function') {
      window.XBBridge.send = function(options) {
        return window.XBBridge.generate.generate(options);
      };
    }
    log('ST适配别名已添加: send');
  }
