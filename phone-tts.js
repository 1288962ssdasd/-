...
if (window.phoneTTS) {
    if (!window.phoneTTS.speak && typeof window.phoneTTS.play === 'function') {
        window.phoneTTS.speak = function(text, voiceId, onStateChange) {
            return window.phoneTTS.play(text, voiceId, onStateChange);
        };
    }
    console.log('[PhoneTTS] ✅ ST适配别名已添加: speak');
}
