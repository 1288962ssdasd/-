...
if (typeof window.MessageApp === 'undefined') {
  class MessageApp {
    ...
    updateMessageListIncrementally() {
      try {
        ...
        if (!messageListContainer) {
          console.warn('[Message App] 找不到消息列表容器，当前页面可能不在消息应用中');
          return;
        }
    ...
    // [修复v4] ★ 关键修复：事件处理时直接从 PhoneDataStore 拉取数据更新 friendsData
    _setupBridgeEventListeners() {
      ...
      function trySetup() {
        ...
        // ★ 辅助函数：从 PhoneDataStore 拉取数据更新 friendsData
        function syncFriendsDataFromStore(friendId, friendName) {
          if (!window.PhoneDataStore || !friendId) {
            console.warn('[Message App] syncFriendsDataFromStore: PhoneDataStore 或 friendId 不可用');
            return false;
          }
          
          // 初始化 friendsData
          if (!self.friendsData) self.friendsData = {};
          
          // 从 PhoneDataStore 读取消息数据
          var messages = window.PhoneDataStore.get('messages.' + friendId);
          var lastMessage = window.PhoneDataStore.get('lastMessage.' + friendId);
          
          // 初始化或更新好友条目
          if (!self.friendsData[friendId]) {
            self.friendsData[friendId] = {
          ...
          // 更新消息列表
          if (messages) {
            self.friendsData[friendId].messages = Array.isArray(messages) ? messages : [messages];
          }
          if (lastMessage) {
            self.friendsData[friendId].lastMessage = lastMessage;
          }
          
          console.log('[Message App] ★ friendsData 已从 PhoneDataStore 同步:', friendId, 
            '消息数:', self.friendsData[friendId].messages.length);
          return true;
        }
        ...
        EventBus.on('phone:friend_added', function(data) {
          ...
          var friendId = data && (data.friendId || data.id || data.name);
          var friendName = data && (data.friendName || data.name);
          syncFriendsDataFromStore(friendId, friendName);
          setTimeout(function() {
            // ★ 修复：在消息详情页时不重新渲染（会打断异步加载），只刷新列表
            if (self.currentView === 'messageDetail') {
              console.log('[Message App] 当前在消息详情页，跳过 updateAppContent');
              return;
            }
        ...
        EventBus.on('phone:notification', function(data) {
          ...
          var friendId = data && (data.friendId || data.fromId || (data.data && data.data.friendId));
          var friendName = data && (data.friendName || data.from || (data.data && data.data.friendName));
          syncFriendsDataFromStore(friendId, friendName);
          setTimeout(function() {
            // ★ 修复：在消息详情页时不重新渲染（会打断异步加载），只刷新列表
            if (self.currentView === 'messageDetail') {
        ...
        EventBus.on('phone:message_sent', function(data) {
          ...
          var friendId = data && (data.friendId || data.toId || (data.data && data.data.friendId));
          var friendName = data && (data.friendName || data.to || (data.data && data.data.friendName));
          syncFriendsDataFromStore(friendId, friendName);
          setTimeout(function() {
            // ★ 修复：消息发送后需要刷新详情页（新消息应显示）
            if (self.updateAppContent) {
              try { self.updateAppContent(); } catch(e) {}
            }
          }, 200);
    ...
    // 渲染消息详情页面
    renderMessageDetail() {
      console.log('[Message App] renderMessageDetail 被调用: friendId=' + this.currentFriendId + ', friendName=' + this.currentFriendName);

      // ★ 修复：如果 currentFriendId 为 null，尝试从 appStack 恢复
      if (!this.currentFriendId && window.mobilePhone && window.mobilePhone.appStack) {
        var stack = window.mobilePhone.appStack;
        for (var i = stack.length - 1; i >= 0; i--) {
          if (stack[i].view === 'messageDetail' && stack[i].friendId) {
            console.log('[Message App] ★ 从 appStack 恢复 friendId:', stack[i].friendId);
            this.currentFriendId = stack[i].friendId;
            this.currentFriendName = stack[i].friendName;
            break;
          }
        }
      }
      ...
      if (!this.currentFriendId) {
        console.error('[Message App] 没有选中的好友，currentFriendId 为 null');
