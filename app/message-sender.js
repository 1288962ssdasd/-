...
if (typeof window.MessageSender === 'undefined') {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // [ST适配] 添加方法别名
      if (window.messageSender && !window.messageSender.send && typeof window.messageSender.sendMessage === 'function') {
        window.messageSender.send = function(message) {
          return window.messageSender.sendMessage(message);
        };
        console.log('[MessageSender] ✅ ST适配别名已添加: send');
      }
    });
  } else {
    window.messageSender = new MessageSender();
