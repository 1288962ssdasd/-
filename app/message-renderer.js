...
if (typeof window.MessageRenderer === 'undefined') {
  ...
  if (window.messageRenderer) {
    if (!window.messageRenderer.render && typeof window.messageRenderer.renderSingleMessage === 'function') {
      window.messageRenderer.render = function(message) {
        return window.messageRenderer.renderSingleMessage(message);
      };
    }
    if (!window.messageRenderer.renderMessages && typeof window.messageRenderer.renderMessagesBatch === 'function') {
      window.messageRenderer.renderMessages = function(messages) {
        return window.messageRenderer.renderMessagesBatch(messages);
      };
    }
    console.log('[MessageRenderer] ✅ ST适配别名已添加: render, renderMessages');
  }
