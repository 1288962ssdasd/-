...
class MobilePhone {
    ...
    async returnToForumMainList() {
        ...
        if (window.getForumAppContent && window.bindForumEvents) {
            ...
            // 获取完整的论坛应用HTML
            const forumContent = await window.getForumAppContent();
            if (forumContent && typeof forumContent === 'string') {
                // 设置到应用内容区域
    ...
    async handleForumApp() {
        try {
            ...
            if (view === 'forumControl') {
                ...
                content = await window.getForumControlAppContent();
            } else {
                // 显示主论坛界面
                ...
                content = await window.getForumAppContent();
            }

            // 类型检查：确保 content 是字符串
            if (content && typeof content !== 'string') {
                console.warn('[Mobile Phone] content 类型异常:', typeof content, content);
...
现在，你已经理解了你的任务。请准备好，等待用户输入**[论坛主题或社群名称]**。`;
    ...
    initializeParallelEventsStyleSelector() {
        ...
        // 等待平行事件样式管理器初始化完成（最多重试20次，2秒超时）
        let _styleInitRetries = 0;
        const initializeSelector = () => {
            if (!window.parallelEventsStyles) {
                _styleInitRetries++;
                if (_styleInitRetries > 20) {
                    console.warn('[Mobile Phone] 平行事件样式管理器初始化超时，跳过');
                    return;
                }
    ...
    async loadForumApp() {
        ...
        // [ST适配修复] 如果 phone-loader 已加载了核心模块（forumManager/forumStyles），
        // 但 UI 模块尚未加载，则只加载缺失的 UI 模块，避免重复加载管理器
        var phoneLoaderLoaded = window.Phone && window.Phone.isReady && window.Phone.getLoadOrder;
        var coreLoaded = phoneLoaderLoaded && window.forumManager && window.forumStyles;
        if (coreLoaded) {
            console.log('[Mobile Phone] Forum 核心模块已由 phone-loader 加载，仅补充 UI 模块');
            // 只加载 phone-loader 不负责的 UI 层文件
            var uiFiles = [
                '/scripts/extensions/third-party/mobile/app/forum-app/forum-ui.css',
                '/scripts/extensions/third-party/mobile/app/forum-app/forum-control-app.css',
                '/scripts/extensions/third-party/mobile/app/forum-app/forum-ui.js',
                '/scripts/extensions/third-party/mobile/app/forum-app/forum-control-app.js'
            ];
            var loadPromises = uiFiles.map(function(src) {
                return new Promise(function(resolve) {
                    var isCSS = src.endsWith('.css');
                    var el;
                    if (isCSS) {
                        ...
                        if (document.querySelector('script[src*="' + fileName + '"]')) {
                            console.log('[Mobile Phone] 跳过已加载: ' + fileName);
                            resolve();
                            return;
                        }
                    ...
                    el.onerror = function() { resolve(); }; // 非致命
            ...
            console.log('[Mobile Phone] ✅ Forum UI 补充加载完成');
            return Promise.resolve();
        }
    ...
    async loadWeiboApp() {
        ...
        // [ST适配修复] 如果 phone-loader 已加载了核心模块（weiboManager/weiboStyles），
        // 但 UI 模块尚未加载，则只加载缺失的 UI 模块，避免重复加载管理器
        var phoneLoaderLoaded = window.Phone && window.Phone.isReady && window.Phone.getLoadOrder;
        var coreLoaded = phoneLoaderLoaded && window.weiboManager && window.weiboStyles;
        if (coreLoaded) {
            console.log('[Mobile Phone] Weibo 核心模块已由 phone-loader 加载，仅补充 UI 模块');
            var uiFiles = [
                '/scripts/extensions/third-party/mobile/app/weibo-app/weibo-ui.css',
                '/scripts/extensions/third-party/mobile/app/weibo-app/weibo-control-app.css',
                '/scripts/extensions/third-party/mobile/app/weibo-app/weibo-ui.js',
                '/scripts/extensions/third-party/mobile/app/weibo-app/weibo-control-app.js'
            ];
            var loadPromises = uiFiles.map(function(src) {
                return new Promise(function(resolve) {
                    var isCSS = src.endsWith('.css');
            ...
            await Promise.all(loadPromises);
            await new Promise(function(resolve) { setTimeout(resolve, 500); });
            console.log('[Mobile Phone] ✅ Weibo UI 补充加载完成');
            return Promise.resolve();
        }
    ...
    async loadParallelEventsApp() {
        ...
        window._parallelEventsAppLoading = new Promise((resolve, reject) => {
            ...
            const handleError = name => {
                console.warn(`[Mobile Phone] ${name} 加载失败，平行事件功能不可用`);
                window._parallelEventsAppLoading = null;
                window._parallelEventsAppSkipped = true;
                resolve(); // 优雅降级，不阻塞其他模块
            };
